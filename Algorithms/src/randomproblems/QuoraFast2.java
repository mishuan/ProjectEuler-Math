...
public class QuoraFast2 {
   ...
   private static String findQResults(int numResults, double x, double y) {
      StringBuffer returnString = new StringBuffer();
      Set<Integer> keys = topics.keySet();
      ArrayList<DistancePair> questionArray = new ArrayList<DistancePair>(numResults);
      HashSet<Integer> questionSet = new HashSet<Integer>(numResults);
      for (Integer j : keys) {
         double distance = computeDistance(j, x, y);
         ...
         for (int i = 0; i < count; i++) {
            ...
            if (questionArray.size() < numResults) {
               ...
               if (!questionSet.contains(currQuestion)) {
                  questionSet.remove(questionArray.get(numResults - 1).id);
                  questionArray.remove(numResults - 1);
                  questionSet.add(currQuestion);
                  questionArray.add(qdPair);
               }
            }
