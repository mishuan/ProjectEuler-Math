...
/**
 * This class will be used to for testing and researching encryption techniques,
 * and encrypted matching. Each instance of this class will always contain ONE
 * set of client/server matches.
 * 
 * @author Michael Yuan
 * 
 */
...
public class RandomHexGenerator {
	 ...
	 * 
	 * @param limit
	 *            number of strings in the hexadecimal array being used for
	 *            matching
	 * @param matchLimit
	 *            number of strings in the array being matched against
	 * @param length
	 *            length of every string
	 * @param numOfMatches
	 *            number of strings that should be matched between matchStrings
	 *            and hexStrings
	 * @throws Exception
	 */
	public RandomHexGenerator(int limit, int matchLimit, int length,
			int numOfMatches) throws Exception {
	 ...
	 * @throws Exception
	 */
	public void reconstruct(int limit, int matchLimit, int length,
			int numOfMatches) throws Exception {
	 ...
	 * 
	 * @param directory
	 *            the directory where the strings are contained. follows the
	 *            format: /some/absolute/path/myfile.ext
	 * @param option
	 *            select 0 or 1 for which array desired to be printed. 0 is
	 *            client array, 1 is match array
	 */
	public void writeArrayStrings(String directory, int option)
			throws IOException {
		int size;
		String[] array;
		if (option == 0) {
	...
	/**
	 * Method used to randomly generate number of hex strings. This method is
	 * used private matching
	 * 
	 * @param limit
	 *            Number of strings desired in the array
	 * @param length
	...
	/**
	 * Method used to randomly generated an array with some matches to
	 * hexStrings array. Note the desired matches is the optimal number of
	 * matches. In reality the number of matches can be smaller if the random
	 * generator generates the same number more than once (number of matches
	 * will follow a normal distribution)
	 * 
	 * @param stringArray
	 *            the array being matched with
	 * @param desiredMatches
	 *            number of matches desired to the original array
	 * @return a random array containing certain number of entries matching
	 *         original array
	 * @throws Exception
	 */
	private String[] matchStrings(String[] stringArray, int limit,
			int desiredMatches) throws Exception {
		if (limit <= 0) {
			throw new IllegalArgumentException();
		}
		if (desiredMatches == 0) {
